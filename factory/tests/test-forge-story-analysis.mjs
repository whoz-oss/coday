...
mkdirSync(join(repo,'apps')); writeFileSync(join(repo,'apps','existing.ts'),'export {}')
...
const fake={preflightAgent:async()=>({ok:true,agent:{integrations:{FACTORY_FILES_RO:null,QUERY_USER:[]}}}),preflightReadOnlyWorkspace:async()=>({ok:true,rootPath:repo}),createCase:async()=>({id:'case-1'}),runAgentTurn:async()=>({status:'finished',caseStatus:'IDLE',message:'```json\n{"files":["apps/existing.ts"],"doneWhen":"implemented","steps":["change"]}\n```',killedByBudget:false})}
await assert.rejects(()=>executeStoryAnalysis({roots,epicRunId:run.runId,storyRunId:storyId,namespaceId:'ns',agentName:'analyst',runtime:fake}),/G1_NOT_APPROVED/)
...
const result=await executeStoryAnalysis({roots,epicRunId:run.runId,storyRunId:storyId,namespaceId:'ns',agentName:'analyst',expectedSpecHash:hash,runtime:fake}); assert.equal(result.outcome,'finished')
...
const unsafe={...fake,preflightReadOnlyWorkspace:async()=>({ok:false,reason:'readOnly:false'})}; await assert.rejects(()=>executeStoryAnalysis({roots,epicRunId:run.runId,storyRunId:storyId,namespaceId:'ns',agentName:'analyst',expectedSpecHash:hash,runtime:unsafe}),/READ_ONLY_PREFLIGHT_FAILED/)
const failing={...fake,runAgentTurn:async()=>({status:'work_timeout',caseStatus:null,killedByBudget:true,message:'secret prose'})}; const failed=await executeStoryAnalysis({roots,epicRunId:run.runId,storyRunId:storyId,namespaceId:'ns',agentName:'analyst',expectedSpecHash:hash,runtime:failing}); assert.equal(failed.execution.status,'failed')
