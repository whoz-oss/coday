...
try {
  ...
  expect('lookup existing is namespace scoped', [(await store.lookup('namespace-1', 'wf-1')).state, (await store.lookup('namespace-2', 'wf-1')).state], ['existing', 'absent'])
  expect('lookup preserves v1 snapshot', (await store.lookup('namespace-1', 'wf-1')).snapshot.projection, changed.snapshot.projection)
  const v2 = { schemaVersion: '2', workflowId: 'wf-v2', workflowType: 'delivery-v2', title: 'V2', status: 'ready', steps: [{ id: 'owned', name: 'Owned', status: 'ready', responsibility: { kind: 'agent', name: 'Sway' } }] }
  await store.publish('namespace-1', v2)
  expect('lookup preserves v2 snapshot', (await store.lookup('namespace-1', 'wf-v2')).snapshot.projection, v2)
  await store.remove('namespace-1', 'wf-v2')
  expect('lookup removed', (await store.lookup('namespace-1', 'wf-v2')).state, 'removed')
  await store.purge('namespace-1', 'wf-v2')
  expect('lookup purged', (await store.lookup('namespace-1', 'wf-v2')).state, 'purged')
