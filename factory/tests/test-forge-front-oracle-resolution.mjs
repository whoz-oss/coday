...
const effectiveTargets={aphrodite:{build:{}},admin:{build:{}},'agentic-studio':{build:{}},'copilot-chat':{build:{}},'entity-list-base':{test:{}},'sprint-entity-list-base':{test:{}},'without-test':{}}
let inspections=[]
const projectInspector=(name)=>{inspections.push(name);return {name,targets:effectiveTargets[name]??{}}}
...
let plan=resolveFrontOraclePlan({repoRoot:root,files:[file('entity-list-base')],hostMapRaw:hostMap,buildTemplate:build,projectInspector})
...
plan=resolveFrontOraclePlan({repoRoot:root,files:[file('without-test','tests-only.ts')],buildTemplate:build,requireBuild:false,projectInspector})
...
plan=resolveFrontOraclePlan({repoRoot:root,files:[file('entity-list-base','test.ts')],hostMapRaw:hostMap,buildTemplate:build,projectInspector})
...
plan=resolveFrontOraclePlan({repoRoot:root,files:['frontend/libs/shared/entity-list-base/source.ts'],hostMapRaw:hostMap,buildTemplate:build,testsTarget:'test',projectInspector})
...
assert.throws(()=>resolveFrontOraclePlan({repoRoot:root,files:['frontend/libs/shared/entity-list-base/source.ts','frontend/libs/shared/duplicate-entity-list-base/source.ts'],hostMapRaw:hostMap,buildTemplate:build,testsTarget:'test',projectInspector}),error=>error.code==='ORACLE_INFRASTRUCTURE')
...
plan=resolveFrontOraclePlan({repoRoot:root,files:[file('entity-list-base','mixed.ts'),file('without-test')],hostMapRaw:hostMap,buildTemplate:build,projectInspector})
assert.deepEqual(plan.tests.ownersWithTestTarget,['entity-list-base'])
assert.deepEqual(plan.tests.ownersWithoutTestTarget,['without-test'])
assert.match(plan.tests.command,/--projects=entity-list-base/)
assert.doesNotMatch(plan.tests.command,/without-test/)

// Effective targets are authoritative even when a raw project.json omits inferred targets.
inspections=[];plan=resolveFrontOraclePlan({repoRoot:root,files:[file('entity-list-base','again.ts'),file('entity-list-base','again-2.ts')],hostMapRaw:hostMap,buildTemplate:build,testsTarget:'test',projectInspector})
assert.match(plan.tests.command,/entity-list-base/)
assert.equal(inspections.filter(name=>name==='entity-list-base').length,1)
for(const inspector of [()=>{throw new Error('non-zero secret stdout')},()=>{throw Object.assign(new Error('timeout secret stderr'),{code:'ETIMEDOUT'})},()=>({name:'entity-list-base',targets:[]}),()=>({name:'other',targets:{test:{}}})])assert.throws(()=>resolveFrontOraclePlan({repoRoot:root,files:[file('entity-list-base','inspect-failure.ts')],hostMapRaw:hostMap,buildTemplate:build,testsTarget:'test',projectInspector:inspector}),error=>error.code==='ORACLE_INFRASTRUCTURE'&&!error.message.includes('secret'))
