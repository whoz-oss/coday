...
export async function handleWorkflowEvidenceRequest({method,path,url,readBody,send,projectionStore,evidenceStore,definitionRegistry,log=console}){
 ...
 if(method==='POST'){
  ...
  if(validation.value.kind==='oracle-result'){sendError(send,403,'FACTORY_ONLY_EVIDENCE','oracle-result evidence is produced only by the Factory oracle runner.');return true}
