...
export async function handleDeliveryOperationRequest({method,path,readBody,send,identity,controller,log=console}){
 ...
 try{const trust=await identity();if(!trust){sendError(send,401,'TRUST_CONTEXT_UNAVAILABLE');return true}const workflowId=decodeURIComponent(match[1]),action=match[2],requestId=match[3],sub=match[4];let result
 ...
 }catch(error){log.error('Delivery operation control-plane failure',{code:error?.code??'UNEXPECTED'});sendError(send,500,'DELIVERY_OPERATION_CONTROL_PLANE_FAILURE');return true}
}
