...
export async function validateOracleRoot(repoRoot) {
...
export function oracleRootIdentity(repoRoot) {
...
export function executeOracle(
  definition,
  { repoRoot, countTaskOutcomes, spawnImpl = spawn, environment = process.env }
) {
  return new Promise((resolve) => {
    ...
    child.on('close', (code, signal) => {
      ...
      const out = bounded(stdout),
        err = bounded(stderr),
        counts = countTaskOutcomes(`${out.excerpt}\n${err.excerpt}`)
      const base = {
        exitCode: code,
        signal,
        timedOut,
...
export function oracleArtifact(result) {
