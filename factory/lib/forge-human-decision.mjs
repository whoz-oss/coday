 ...
 */
export async function recordHumanDecision({ roots, runId, decision, identityPort, now = () => new Date().toISOString() }) {
  if (!identityPort || typeof identityPort.authorize !== 'function') throw new Error('an identity authorization port is required')
  ...
  if (decision.actorId !== undefined || decision.actorRole !== undefined) throw new Error('actor identity and role must not be declared by the decision payload')
  ...
  if (decision.gate !== 'G1' || decision.attempt !== gate.attempt || decision.policyVersion !== G1_POLICY_VERSION) throw new Error('decision does not match the active G1 attempt or policy')
  ...
  const authority = await identityPort.authorize({ actorId, runId, gate: 'G1', attempt: gate.attempt, policyVersion: G1_POLICY_VERSION })
  if (!authority || typeof authority.authorityId !== 'string') throw new Error('actor is not authorized to decide G1')

  const existing = events.find((event) => event.event === 'human_decision_recorded' && event.runId === runId && event.gate === 'G1' && event.attempt === gate.attempt)
  const fingerprint = canonical({ outcome: decision.outcome, reasonCode: decision.reasonCode, evidenceSetHash, actorId, authorityId: authority.authorityId })
  if (existing) {
  ...
  const event = {
    ...
    evidenceSetHash,
    decision: { actorId, authorityId: authority.authorityId, outcome: decision.outcome, reasonCode: decision.reasonCode },
