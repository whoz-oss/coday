...
export class DeliveryGitControlPlane {
  ...
  compareClaims(inspection, claims) {
    if (
      !claims ||
      Object.keys(claims).some((key) => !['paths', 'diffHash'].includes(key)) ||
      !Array.isArray(claims.paths)
    )
      fail('INVALID_CLAIMS')
    const actual = [...new Set(inspection.files.map((item) => item.path))].sort(),
      claimed = [...new Set(claims.paths)].sort()
    if (JSON.stringify(actual) !== JSON.stringify(claimed) || claims.diffHash !== inspection.diffHash)
