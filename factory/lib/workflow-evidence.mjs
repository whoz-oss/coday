...
export const WORKFLOW_EVIDENCE_KINDS = Object.freeze(['agent-result', 'artifact', 'oracle-result'])
...
const FACT_KEYS = new Set(['resultCode', 'category', 'attempt', 'durationMs', 'itemCount', 'oracleId', 'oracleVersion', 'oracleHash', 'commandId', 'cwdId', 'exitCode', 'signal', 'timedOut', 'classification', 'executed', 'fromCache', 'upToDate', 'skipped', 'outputHash', 'outputTruncated'])
...
export function validateWorkflowEvidenceInput(input, expectedWorkflowId) {
  ...
  if (input.kind === 'artifact') {
    ...
    if (input.kind === 'oracle-result' && input.outcome === undefined) return invalid('outcome')
