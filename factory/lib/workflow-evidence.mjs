...
export function validateWorkflowEvidenceInput(input, expectedWorkflowId) {
  ...
  if (input.idempotencyKey !== undefined && !boundedText(input.idempotencyKey, WORKFLOW_EVIDENCE_LIMITS.idempotencyKey))
  ...
  if (input.kind === 'artifact') {
    ...
    if (['oracle-result', 'human-decision'].includes(input.kind) && input.outcome === undefined)
  ...
  return {
    ...
    value: {
      ...
      ...(input.kind === 'artifact'
        ? { artifactRef: input.artifactRef, artifactHash: input.artifactHash }
        : { facts: { ...input.facts } }),
