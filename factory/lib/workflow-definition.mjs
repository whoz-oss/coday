...
export function validateWorkflowDefinition(input) {
  ...
  if (Object.keys(input).some((field) => !DEFINITION_FIELDS.has(field)))
    return failure(WORKFLOW_DEFINITION_ERROR_CODES.INVALID_VALUE, '$', { reason: 'unknown_field' })
  if (input.schemaVersion !== WORKFLOW_DEFINITION_SCHEMA_VERSION)
    return failure(WORKFLOW_DEFINITION_ERROR_CODES.INVALID_SCHEMA_VERSION, 'schemaVersion')
  const type = text(input.workflowType, 'workflowType', { safe: true })
  ...
  for (let index = 0; index < input.steps.length; index++) {
    ...
    const id = text(raw.id, `${base}.id`, { safe: true })
    if (!id.ok) return id
    if (ids.has(raw.id))
      return failure(WORKFLOW_DEFINITION_ERROR_CODES.DUPLICATE_STEP_ID, `${base}.id`, { stepId: raw.id })
    ids.add(raw.id)
    const name = text(raw.name, `${base}.name`)
    ...
    if (
      ...
      !KINDS.has(responsibility.kind)
    )
      return failure(WORKFLOW_DEFINITION_ERROR_CODES.INVALID_RESPONSIBILITY, `${base}.responsibility`)
    const responsibilityName = text(responsibility.name, `${base}.responsibility.name`)
    ...
    for (let dependencyIndex = 0; dependencyIndex < raw.dependsOn.length; dependencyIndex++) {
      const dependency = text(raw.dependsOn[dependencyIndex], `${base}.dependsOn[${dependencyIndex}]`, { safe: true })
    ...
    steps.push({
      id: raw.id,
      name: raw.name,
      responsibility: { kind: responsibility.kind, name: responsibility.name },
