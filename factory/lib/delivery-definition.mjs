...
export function validateDeliveryDefinition(input) {
  ...
  if (
    ...
    return fail('$')
  if (!Array.isArray(input.checkpoints) || input.checkpoints.length !== DELIVERY_STAGES.length)
  ...
  for (let index = 0; index < input.checkpoints.length; index++) {
    ...
    if (
      !raw.responsibility ||
      Object.keys(raw.responsibility).some((key) => !RESPONSIBILITY.has(key)) ||
      !['code', 'human'].includes(raw.responsibility.kind) ||
      !SAFE.test(raw.responsibility.name ?? '')
    )
      return fail(`checkpoints[${index}].responsibility`)
    if (raw.stage === 'release-approved' && raw.responsibility.kind !== 'human')
      return fail(`checkpoints[${index}].responsibility`, 'release_requires_human')
    if (raw.stage !== 'release-approved' && raw.responsibility.kind !== 'code')
    ...
    for (let evidenceIndex = 0; evidenceIndex < raw.requiredEvidence.length; evidenceIndex++) {
      ...
      if (
        ...
        !DELIVERY_EVIDENCE_KINDS.includes(item.kind) ||
        !['pass', 'fail', 'indeterminate', 'approved', 'rejected'].includes(item.outcome)
      )
        return fail(`checkpoints[${index}].requiredEvidence[${evidenceIndex}]`)
      if (item.oracleId !== undefined && !SAFE.test(item.oracleId))
