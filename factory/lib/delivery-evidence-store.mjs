...
export function validateDeliveryEvidence(input) {
  ...
  if (
    ![input.deliveryId, input.workflowId, input.runtimeId, input.kind, input.outcome, input.idempotencyKey].every(
      (value) => SAFE.test(value ?? '')
    ) ||
    !HASH.test(input.environmentHash ?? '') ||
    !UUID.test(input.caseId ?? '') ||
    !SHA.test(input.headCommit ?? '')
  )
    return { ok: false, error: { code: 'INVALID_DELIVERY_EVIDENCE' } }
  if (
    !input.facts ||
    ...
    Object.keys(input.facts).length > 32 ||
    JSON.stringify(input.facts).length > 4096
  )
    return { ok: false, error: { code: 'INVALID_DELIVERY_EVIDENCE' } }
...
export class DeliveryEvidenceStore {
  ...
  async list(namespaceId, deliveryId) {
    try {
      return (await readFile(this.path(namespaceId, deliveryId), 'utf8')).split('\n').filter(Boolean).map(JSON.parse)
    } catch (error) {
  ...
  async record(namespaceId, input, source) {
    ...
    return this._locked(`${namespaceId}\0${input.deliveryId}`, async () => {
      const existing = await this.list(namespaceId, input.deliveryId),
        scopeHash = digest({
          namespaceId,
        ...
        semanticHash = digest({ ...input, idempotencyKey: undefined })
      const prior = existing.find((item) => item.idempotency.scopeHash === scopeHash)
      if (prior)
        return prior.idempotency.semanticHash === semanticHash
      ...
      await append(this.path(namespaceId, input.deliveryId), evidence)
