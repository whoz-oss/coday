...
} from './delivery-operation-definition.mjs'
...
const validSnapshot = (value) =>
  ...
  UUID.test(value.namespaceId ?? '') &&
  SAFE.test(value.deliveryId ?? '') &&
  SAFE.test(value.workflowId ?? '') &&
  UUID.test(value.environmentId ?? '') &&
  HASH.test(value.environmentHash ?? '') &&
  UUID.test(value.parentCaseId ?? '') &&
  SAFE.test(value.runtimeId ?? '') &&
  SHA.test(value.baseCommit ?? '') &&
  SHA.test(value.headCommit ?? '') &&
...
export class DeliveryStore {
  ...
  async _write(current, value, operationInput) {
    const paths = this.paths(value.namespaceId, value.deliveryId),
    ...
    const snapshot = { ...clean, snapshotHash: hash(clean) }
  ...
  _deliveryOperationProjection(records) {
    ...
    const resolved = new Set(
      history
        .filter((record) => record.resolvedOperationId && ['succeeded', 'failed'].includes(record.state))
        .map((record) => record.resolvedOperationId)
    )
