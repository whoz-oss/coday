...
export class WorkflowDefinitionRegistry {
  ...
  async list() {
    return [...(await this.#load()).values()].sort(
      (a, b) =>
        a.workflowType.localeCompare(b.workflowType) || a.version.localeCompare(b.version, undefined, { numeric: true })
    )
  }
  ...
  async resolveUnique(workflowType) {
    ...
    if (matches.length === 0)
      throw new WorkflowDefinitionRegistryError('WORKFLOW_DEFINITION_NOT_FOUND', { workflowType })
  ...
  async #load() {
    ...
    let types = []
    try {
      types = await readdir(this.root, { withFileTypes: true })
    } catch (error) {
    ...
    for (const typeEntry of types.filter((entry) => entry.isDirectory()).sort((a, b) => a.name.localeCompare(b.name))) {
      ...
      const files = (await readdir(typeRoot, { withFileTypes: true }))
        .filter((entry) => entry.isFile() && entry.name.endsWith('.json'))
        .sort((a, b) => a.name.localeCompare(b.name))
      for (const file of files) {
        let parsed
        try {
          parsed = JSON.parse(await readFile(join(typeRoot, file.name), 'utf8'))
        } catch {
          throw new WorkflowDefinitionRegistryError('INVALID_DEFINITION_FILE', {
            workflowType: typeEntry.name,
            file: file.name,
          })
        }
        const validated = validateWorkflowDefinition(parsed)
        if (!validated.ok)
          throw new WorkflowDefinitionRegistryError(validated.error.code, { ...validated.error, file: file.name })
        const expectedVersion = file.name.slice(0, -5)
        if (validated.definition.workflowType !== typeEntry.name || validated.definition.version !== expectedVersion)
          throw new WorkflowDefinitionRegistryError('DEFINITION_PATH_MISMATCH', {
            workflowType: typeEntry.name,
        ...
        if (existing && existing.definitionHash !== definitionHash)
          throw new WorkflowDefinitionRegistryError('DEFINITION_COLLISION', { key })
