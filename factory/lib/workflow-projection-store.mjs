...
export class WorkflowProjectionStoreError extends Error {
  constructor(code, details = {}, cause) { super(code, cause ? { cause } : undefined); this.name = 'WorkflowProjectionStoreError'; this.code = code; this.details = details }
}

function storageId(namespaceId, workflowId) { return createHash('sha256').update(`${namespaceId}:${workflowId}`, 'utf8').digest('hex') }
...
async function atomicJsonWrite(filePath, value) {
  ...
  await rename(temporary, filePath); await syncDirectory(dirname(filePath))
}
...
export class WorkflowProjectionStore {
  constructor(dataRoot, { lifecycleFault = async () => {} } = {}) { if (typeof dataRoot !== 'string' || dataRoot.length === 0) throw new WorkflowProjectionStoreError(WORKFLOW_STORE_ERROR_CODES.INVALID_DATA_ROOT); this.dataRoot = dataRoot; this.locks = new Map(); this.lifecycleFault = lifecycleFault }
  ...
  paths(namespaceId, workflowId) {
    ...
    const workflow = validateWorkflowProjectionId(workflowId, 'workflowId'); if (!workflow.ok) throw new WorkflowProjectionStoreError(WORKFLOW_STORE_ERROR_CODES.CORRUPT_STORAGE, workflow.error)
    const id = storageId(namespaceId, workflowId); const directory = join(this.dataRoot, 'workflows', namespaceId, id); const trashDirectory = join(this.dataRoot, 'trash', id)
    return { storageId: id, directory, snapshot: join(directory, 'projection.json'), events: join(directory, 'events.jsonl'), pending: join(directory, 'pending.json'), trashDirectory, trashSnapshot: join(trashDirectory, 'projection.json'), trashEvents: join(trashDirectory, 'events.jsonl'), tombstone: join(this.dataRoot, 'tombstones', `${id}.json`) }
  }

  async initialize() { await mkdir(join(this.dataRoot, 'workflows'), { recursive: true }); await mkdir(join(this.dataRoot, 'trash'), { recursive: true }); await mkdir(join(this.dataRoot, 'tombstones'), { recursive: true }) }
  _locked(namespaceId, workflowId, action) { const key = `${namespaceId}\u0000${workflowId}`; const prior = this.locks.get(key) ?? Promise.resolve(); const operation = prior.then(action); const tail = operation.catch(() => {}); this.locks.set(key, tail); return operation.finally(() => { if (this.locks.get(key) === tail) this.locks.delete(key) }) }
  async _readJson(path, missing = null) { try { return JSON.parse(await readFile(path, 'utf8')) } catch (error) { if (error?.code === 'ENOENT') return missing; throw new WorkflowProjectionStoreError(WORKFLOW_STORE_ERROR_CODES.CORRUPT_STORAGE, {}, error) } }
  _validateTombstone(value, paths) {
    if (!value || typeof value !== 'object' || Array.isArray(value) || Object.keys(value).some((key) => !['namespaceId','workflowId','storageId','removedAt','removedBy','actorId','generation','lifecycleState','purgedAt','purgedBy'].includes(key)) || value.namespaceId === undefined || value.workflowId === undefined || value.storageId !== paths.storageId || value.storageId !== storageId(value.namespaceId, value.workflowId) || !Number.isSafeInteger(value.generation) || value.generation < 1 || !['removing','removed','purged'].includes(value.lifecycleState) || (value.lifecycleState === 'purged') !== (value.purgedAt !== undefined) || Number.isNaN(Date.parse(value.removedAt)) || (value.purgedAt !== undefined && Number.isNaN(Date.parse(value.purgedAt)))) throw new WorkflowProjectionStoreError(WORKFLOW_STORE_ERROR_CODES.CORRUPT_STORAGE)
    return value
  }
  async _tombstone(paths) { const value = await this._readJson(paths.tombstone); return value ? this._validateTombstone(value, paths) : null }
  async _readSnapshot(paths) { try { const snapshot = JSON.parse(await readFile(paths.snapshot, 'utf8')); if (!Number.isSafeInteger(snapshot.revision) || snapshot.revision < 1 || hashWorkflowProjection(snapshot.projection) !== snapshot.projectionHash) throw new Error('invalid snapshot'); return snapshot } catch (error) { if (error?.code === 'ENOENT') return null; throw new WorkflowProjectionStoreError(WORKFLOW_STORE_ERROR_CODES.CORRUPT_STORAGE, {}, error) } }
  ...
  async _listRoot(namespaceId, removed) {
    const namespace = validateWorkflowProjectionId(namespaceId, 'namespaceId'); if (!namespace.ok) throw new WorkflowProjectionStoreError(WORKFLOW_STORE_ERROR_CODES.INVALID_NAMESPACE_ID, namespace.error)
    if (!removed) { const root = join(this.dataRoot, 'workflows', namespaceId); let entries; try { entries = await readdir(root, { withFileTypes: true }) } catch (error) { if (error?.code === 'ENOENT') return []; throw error } const snapshots = []; for (const entry of entries.filter((item) => item.isDirectory()).sort((a,b) => a.name.localeCompare(b.name))) { const p = { directory: join(root, entry.name), snapshot: join(root, entry.name, 'projection.json'), events: join(root, entry.name, 'events.jsonl'), pending: join(root, entry.name, 'pending.json') }; await this._recover(p); const snapshot = await this._readSnapshot(p); if (snapshot) snapshots.push(snapshot) } return snapshots.sort((a,b) => a.projection.workflowId.localeCompare(b.projection.workflowId)) }
  ...
  async _hasLifecycleFact(eventsPath, kind, generation) {
    let journal; try { journal = await readFile(eventsPath, 'utf8') } catch (error) { if (error?.code === 'ENOENT') return false; throw error }
    try { return journal.split('\n').filter(Boolean).map(JSON.parse).some((fact) => fact.kind === kind && fact.generation === generation) } catch (error) { throw new WorkflowProjectionStoreError(WORKFLOW_STORE_ERROR_CODES.CORRUPT_STORAGE, {}, error) }
  }
  async _remove(namespaceId, workflowId, actor) {
    const paths = this.paths(namespaceId, workflowId)
    try {
      let tombstone = await this._tombstone(paths)
      if (!tombstone) { await this._recover(paths); await this.lifecycleFault('after-recovery', { namespaceId, workflowId }) }
      ...
      if (tombstone && tombstone.lifecycleState !== 'removing') return { ok: false, error: { code: WORKFLOW_STORE_ERROR_CODES.INVALID_LIFECYCLE_TRANSITION } }
      if (!tombstone) {
        tombstone = { namespaceId, workflowId, storageId: paths.storageId, removedAt: new Date().toISOString(), generation: 1, lifecycleState: 'removing', ...attribution(actor, ['removedBy','actorId']) }
        await atomicJsonWrite(paths.tombstone, tombstone); await this.lifecycleFault('after-tombstone', { namespaceId, workflowId })
      }
      if (active && trashed) throw new WorkflowProjectionStoreError(WORKFLOW_STORE_ERROR_CODES.CORRUPT_STORAGE)
      if (!active && !trashed) throw new WorkflowProjectionStoreError(WORKFLOW_STORE_ERROR_CODES.CORRUPT_STORAGE)
      if (active) {
        await rename(paths.directory, paths.trashDirectory); await syncDirectory(dirname(paths.directory)); await syncDirectory(dirname(paths.trashDirectory)); await this.lifecycleFault('after-rename', { namespaceId, workflowId }); trashed = active
      }
      if (!(await this._hasLifecycleFact(paths.trashEvents, 'workflow_removed', tombstone.generation))) {
        await appendDurable(paths.trashEvents, { kind: 'workflow_removed', generation: tombstone.generation, timestamp: tombstone.removedAt, ...attribution(tombstone, ['removedBy','actorId']) })
      }
      await this.lifecycleFault('after-removal-fact', { namespaceId, workflowId })
      const completed = { ...tombstone, lifecycleState: 'removed' }; await atomicJsonWrite(paths.tombstone, completed); await this.lifecycleFault('after-finalize', { namespaceId, workflowId })
      return { ok: true, snapshot: trashed }
    } catch (error) { if (error instanceof WorkflowProjectionStoreError) throw error; throw new WorkflowProjectionStoreError(WORKFLOW_STORE_ERROR_CODES.STORAGE_FAILURE, {}, error) }
  }

  async restore(namespaceId, workflowId, actor = {}) { return this._locked(namespaceId, workflowId, () => this._restore(namespaceId, workflowId, actor)) }
  async _restore(namespaceId, workflowId, actor) {
    const paths = this.paths(namespaceId, workflowId)
    try { const tombstone = await this._tombstone(paths); if (!tombstone || tombstone.lifecycleState !== 'removed') return { ok: false, error: { code: tombstone ? WORKFLOW_STORE_ERROR_CODES.INVALID_LIFECYCLE_TRANSITION : WORKFLOW_STORE_ERROR_CODES.WORKFLOW_NOT_FOUND } }; let active = await this._readSnapshot(paths); const trashed = await this._readJson(paths.trashSnapshot); if (!active && !trashed) throw new WorkflowProjectionStoreError(WORKFLOW_STORE_ERROR_CODES.CORRUPT_STORAGE); if (!active) { await rename(paths.trashDirectory, paths.directory); await syncDirectory(dirname(paths.trashDirectory)); await syncDirectory(dirname(paths.directory)); active = await this._readSnapshot(paths) } await appendDurable(paths.events, { kind: 'workflow_restored', timestamp: new Date().toISOString(), ...attribution(actor, ['restoredBy','actorId']) }); await rm(paths.tombstone); await syncDirectory(dirname(paths.tombstone)); return { ok: true, snapshot: active } } catch (error) { if (error instanceof WorkflowProjectionStoreError) throw error; throw new WorkflowProjectionStoreError(WORKFLOW_STORE_ERROR_CODES.STORAGE_FAILURE, {}, error) }
  }

  async purge(namespaceId, workflowId, actor = {}) { return this._locked(namespaceId, workflowId, () => this._purge(namespaceId, workflowId, actor)) }
  async _purge(namespaceId, workflowId, actor) {
    const paths = this.paths(namespaceId, workflowId)
    try { const tombstone = await this._tombstone(paths); if (!tombstone) return { ok: false, error: { code: WORKFLOW_STORE_ERROR_CODES.WORKFLOW_NOT_FOUND } }; if (!['removed','purged'].includes(tombstone.lifecycleState) || await this._readSnapshot(paths)) return { ok: false, error: { code: WORKFLOW_STORE_ERROR_CODES.INVALID_LIFECYCLE_TRANSITION } }; if (tombstone.lifecycleState !== 'purged' && !(await exists(paths.trashSnapshot))) throw new WorkflowProjectionStoreError(WORKFLOW_STORE_ERROR_CODES.CORRUPT_STORAGE); const updated = tombstone.lifecycleState === 'purged' ? tombstone : { ...tombstone, lifecycleState: 'purged', purgedAt: new Date().toISOString(), ...attribution(actor, ['purgedBy']) }; await atomicJsonWrite(paths.tombstone, updated); await rm(paths.trashDirectory, { recursive: true, force: true }); await syncDirectory(dirname(paths.trashDirectory)); return { ok: true, alreadyPurged: tombstone.lifecycleState === 'purged' } } catch (error) { if (error instanceof WorkflowProjectionStoreError) throw error; throw new WorkflowProjectionStoreError(WORKFLOW_STORE_ERROR_CODES.STORAGE_FAILURE, {}, error) }
  }

  async clearPurgedTombstone(namespaceId, workflowId) { return this._locked(namespaceId, workflowId, async () => { const paths = this.paths(namespaceId, workflowId); const tombstone = await this._tombstone(paths); if (!tombstone) return { ok: false, error: { code: WORKFLOW_STORE_ERROR_CODES.WORKFLOW_NOT_FOUND } }; if (tombstone.lifecycleState !== 'purged' || await exists(paths.trashSnapshot) || await this._readSnapshot(paths)) return { ok: false, error: { code: WORKFLOW_STORE_ERROR_CODES.INVALID_LIFECYCLE_TRANSITION } }; await rm(paths.tombstone); await syncDirectory(dirname(paths.tombstone)); return { ok: true } }) }

  async purgeRemovedBefore(namespaceId, cutoff, actor = {}) {
    const instant = cutoff instanceof Date ? cutoff : new Date(cutoff); if (Number.isNaN(instant.getTime())) return { ok: false, error: { code: 'INVALID_CUTOFF' } }
    const result = { ok: true, examined: 0, eligible: 0, purged: 0, alreadyPurged: 0, results: [] }; let entries
    try { entries = await readdir(join(this.dataRoot, 'tombstones'), { withFileTypes: true }); for (const entry of entries.filter((item) => item.isFile() && item.name.endsWith('.json'))) { const raw = await this._readJson(join(this.dataRoot, 'tombstones', entry.name)); if (!raw || typeof raw.namespaceId !== 'string' || typeof raw.workflowId !== 'string') throw new WorkflowProjectionStoreError(WORKFLOW_STORE_ERROR_CODES.CORRUPT_STORAGE); const paths = this.paths(raw.namespaceId, raw.workflowId); const tombstone = this._validateTombstone(raw, paths); if (raw.namespaceId !== namespaceId) continue; result.examined++; if (tombstone.lifecycleState === 'purged') { result.alreadyPurged++; continue } if (new Date(tombstone.removedAt) >= instant) continue; result.eligible++; const purgeResult = await this.purge(namespaceId, tombstone.workflowId, actor); if (!purgeResult.ok) throw new WorkflowProjectionStoreError(WORKFLOW_STORE_ERROR_CODES.CORRUPT_STORAGE); result.purged++; result.results.push({ workflowId: tombstone.workflowId, purged: true }) } return result } catch (error) { if (error instanceof WorkflowProjectionStoreError) throw error; throw new WorkflowProjectionStoreError(WORKFLOW_STORE_ERROR_CODES.STORAGE_FAILURE, {}, error) }
  }
}

export function workflowProjectionStorageId(namespaceId, workflowId) { return storageId(namespaceId, workflowId) }
