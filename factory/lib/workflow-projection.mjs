...
function boundedString(value, maximum, path, { safe = false, optional = false } = {}) {
  ...
  if (typeof value !== 'string' || !value.trim())
    return failure(
      safe ? WORKFLOW_PROJECTION_ERROR_CODES.INVALID_ID : WORKFLOW_PROJECTION_ERROR_CODES.INVALID_VALUE,
      path
    )
...
export function validateWorkflowProjection(input) {
  ...
  if (input.schemaVersion !== '1' && input.schemaVersion !== '2')
    return failure(WORKFLOW_PROJECTION_ERROR_CODES.INVALID_SCHEMA_VERSION, 'schemaVersion')
  if (Object.keys(input).some((key) => !PROJECTION_FIELDS.has(key)))
    return failure(WORKFLOW_PROJECTION_ERROR_CODES.INVALID_VALUE, '$', { reason: 'unknown_field' })
  const schemaVersion = input.schemaVersion
  for (const [field, limit, safe] of [
  ...
  if (input.steps.length > WORKFLOW_PROJECTION_LIMITS.steps)
    return failure(WORKFLOW_PROJECTION_ERROR_CODES.EXCESSIVE_SIZE, 'steps', {
  ...
  for (let index = 0; index < input.steps.length; index++) {
    ...
    if (!raw || typeof raw !== 'object' || Array.isArray(raw))
      return failure(WORKFLOW_PROJECTION_ERROR_CODES.INVALID_VALUE, base)
    const allowed = schemaVersion === '2' ? STEP_V2_FIELDS : STEP_V1_FIELDS
    if (Object.keys(raw).some((key) => !allowed.has(key)))
      return failure(WORKFLOW_PROJECTION_ERROR_CODES.INVALID_VALUE, base, { reason: 'unknown_field' })
    const id = boundedString(raw.id, WORKFLOW_PROJECTION_LIMITS.id, `${base}.id`, { safe: true })
    if (!id.ok) return id
    if (ids.has(raw.id))
      return failure(WORKFLOW_PROJECTION_ERROR_CODES.DUPLICATE_STEP_ID, `${base}.id`, { stepId: raw.id })
    ids.add(raw.id)
    ...
    if (raw.dependsOn !== undefined && !Array.isArray(raw.dependsOn))
      return failure(WORKFLOW_PROJECTION_ERROR_CODES.INVALID_VALUE, `${base}.dependsOn`)
    const dependencies = raw.dependsOn ?? []
    if (dependencies.length > WORKFLOW_PROJECTION_LIMITS.dependenciesPerStep)
      return failure(WORKFLOW_PROJECTION_ERROR_CODES.EXCESSIVE_SIZE, `${base}.dependsOn`, {
    ...
    for (let i = 0; i < dependencies.length; i++) {
      ...
      if (seen.has(dependency.value))
        return failure(WORKFLOW_PROJECTION_ERROR_CODES.INVALID_VALUE, `${base}.dependsOn[${i}]`, {
    ...
    if (schemaVersion === '2') {
      ...
      if (!actor || typeof actor !== 'object' || Array.isArray(actor))
        return failure(WORKFLOW_PROJECTION_ERROR_CODES.INVALID_VALUE, `${base}.responsibility`)
      if (Object.keys(actor).some((key) => !RESPONSIBILITY_FIELDS.has(key)))
        return failure(WORKFLOW_PROJECTION_ERROR_CODES.INVALID_VALUE, `${base}.responsibility`, {
          reason: 'unknown_field',
        })
      if (!ACTOR_KIND_SET.has(actor.kind))
        return failure(WORKFLOW_PROJECTION_ERROR_CODES.INVALID_VALUE, `${base}.responsibility.kind`)
    ...
    steps.push({
      ...
      ...(raw.dependsOn === undefined ? {} : { dependsOn: [...dependencies] }),
  ...
  for (const step of steps)
    for (const target of step.dependsOn ?? []) {
      if (target === step.id)
        return failure(WORKFLOW_PROJECTION_ERROR_CODES.SELF_DEPENDENCY, `steps.${step.id}.dependsOn`, {
          stepId: step.id,
        })
      if (!ids.has(target))
        return failure(WORKFLOW_PROJECTION_ERROR_CODES.MISSING_DEPENDENCY, `steps.${step.id}.dependsOn`, {
          stepId: step.id,
          target,
        })
    }
  ...
  for (const step of steps)
    if (hasCycle(step.id))
      return failure(WORKFLOW_PROJECTION_ERROR_CODES.DEPENDENCY_CYCLE, 'steps', { stepId: step.id })
  return {
    ok: true,
    projection: {
      schemaVersion,
