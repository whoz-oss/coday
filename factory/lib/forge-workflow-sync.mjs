...
export function sanitizeForgeSyncAttribution(body = {}) {
  if (!body || typeof body !== 'object' || Array.isArray(body) || Object.keys(body).some((key) => !ATTRIBUTION_FIELDS.has(key))) return { ok: false, error: { code: 'INVALID_ATTRIBUTION' } }
  for (const value of Object.values(body)) if (typeof value !== 'string' || !SAFE_ATTRIBUTION.test(value)) return { ok: false, error: { code: 'INVALID_ATTRIBUTION' } }
...
export async function syncForgeWorkflowProjection({ repoRoot, namespaceId, ticketId, attribution = {}, store }) {
  if (typeof repoRoot !== 'string' || !SAFE_FORGE_TICKET_ID.test(ticketId ?? '')) return { ok: false, error: { code: 'INVALID_SYNC_TARGET' } }
  ...
  return { ok: true, changed: published.changed, workflowId: adapted.projection.workflowId, revision: published.snapshot.revision, projectionHash: published.snapshot.projectionHash }
}
