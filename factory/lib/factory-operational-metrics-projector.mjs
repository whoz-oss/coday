import {
  halfOpenIntervalEnvelope,
  halfOpenIntervalUnionDuration,
  mergeHalfOpenIntervals,
} from './interval-aggregation.mjs'
...
function workflowId(snapshot) {
  return snapshot?.projection?.workflowId
}
...
function intervalsFromTimings(items, startField, endField) {
  ...
  for (const item of items) {
    const start = instant(item.timing?.[startField]),
      end = instant(item.timing?.[endField])
...
function cycleTime(items, scope) {
  ...
  const reasons = items.flatMap(({ workflowId: id, timing }) => [
    ...(!timing?.complete
      ? (timing?.incompleteReasons ?? ['timing_projection_incomplete']).map((reason) => `${id}:${reason}`)
      : []),
  ...
  if (!intervals.length)
    return availability({
      complete: false,
      reasons: reasons.length ? reasons : ['cycle_endpoints_unavailable'],
      sources,
    })
  if (scope === 'self') {
    ...
    return availability({
      ...
      complete: reasons.length === 0 && intervals.length === items.length,
      reasons,
      sources,
    })
  }
  const merged = mergeHalfOpenIntervals(intervals),
    envelope = halfOpenIntervalEnvelope(intervals)
  return availability({
    value: {
      semantics: 'calendar_envelope_and_interval_union',
      envelope: {
        startAt: new Date(envelope.start).toISOString(),
        endAt: new Date(envelope.end).toISOString(),
        durationMs: envelope.end - envelope.start,
      },
      unionDurationMs: halfOpenIntervalUnionDuration(intervals),
      mergedIntervals: merged.map(({ start, end }) => ({
...
function reviewTime(interactions, observedAt) {
  ...
  for (const interaction of interactions) {
    ...
    if (opened === null) {
      reasons.push(`${interaction.interactionId ?? 'unknown'}:invalid_opened_at`)
      continue
    }
    if (interaction.status === 'replied') {
      const replied = instant(interaction.repliedAt)
      if (replied === null || replied <= opened)
        reasons.push(`${interaction.interactionId ?? 'unknown'}:invalid_replied_at`)
      else completed.push({ start: opened, end: replied })
    } else if (interaction.status === 'open')
      unresolved.push({ interactionId: interaction.interactionId, openedAt: new Date(opened).toISOString() })
  ...
  const value = {
    ...
    durationMs: halfOpenIntervalUnionDuration(completed),
    mergedIntervals: merged.map(({ start, end }) => ({
      startAt: new Date(start).toISOString(),
      endAt: new Date(end).toISOString(),
      durationMs: end - start,
    })),
    completedInteractionCount: completed.length,
  ...
  if (unresolved.length) {
    ...
    value.inProgressObservedApprovalTime =
      observedMs === null
        ? { available: false, reason: 'invalid_observed_at' }
        : {
            available: true,
            semantics: 'open_approval_intervals_closed_at_observed_at',
            observedAt: new Date(observedMs).toISOString(),
            durationMs: halfOpenIntervalUnionDuration(
              unresolved
                .map((item) => ({ start: instant(item.openedAt), end: observedMs }))
...
function currentWip(items) {
  ...
  for (const { workflowId: id, snapshot, timing } of items) {
    ...
    if (!state) {
      reasons.push(`${id}:current_state_unavailable`)
      continue
    }
    if (!WIP.has(state) && !TERMINAL.has(state)) {
      reasons.push(`${id}:unknown_current_state`)
      continue
    }
  ...
  return availability({
    value: {
      ...
      nonTerminalStates: [...WIP],
      count: Object.values(byState).reduce((sum, count) => sum + count, 0),
      byState,
    },
    complete: reasons.length === 0,
    reasons,
    sources: ['workflow-snapshot', 'workflow-journal'],
  })
}
...
function successfulPromotions(deliveries, includedIds, evidence) {
  ...
  for (const delivery of deliveries) {
    ...
    for (const operation of delivery.operations ?? []) {
      ...
      grouped.set(operation.operationId, records)
    }
    for (const [operationId, records] of grouped) {
      ...
      results.push({
        operationId,
        deliveryId: delivery.snapshot.deliveryId,
        workflowId: delivery.snapshot.workflowId,
...
function deliveryStage(records, stage, requiredKind, requiredOutcome) {
  ...
  const matching = records.filter(
    (record) =>
      ...
      record.evidence.some((item) => item.kind === requiredKind && item.outcome === requiredOutcome)
  )
...
function deliveryMetrics(deliveries, evidence, includedIds) {
  ...
  for (const promotion of promotions) {
    const records = byDelivery.get(promotion.deliveryId) ?? []
    records.push(promotion)
    byDelivery.set(promotion.deliveryId, records)
  }
  const intervals = [],
    leadIntervals = [],
    reasons = []
  for (const delivery of deliveries.filter((item) => includedIds.has(item.snapshot?.workflowId))) {
    ...
    if (!release) {
      reasons.push(`${delivery.snapshot.deliveryId}:release_approval_proof_unavailable`)
      continue
    }
    if (!endpoint || endpoint.timestamp <= release.timestamp) {
      reasons.push(`${delivery.snapshot.deliveryId}:successful_delivery_endpoint_unavailable`)
      continue
    }
    intervals.push({
      start: release.timestamp,
      end: endpoint.timestamp,
      endpoint: verified ? 'production-verified' : 'deployed',
      deliveryId: delivery.snapshot.deliveryId,
    })
    const created = instant(delivery.snapshot.createdAt)
    if (created !== null && verified && verified.timestamp > created)
      leadIntervals.push({ start: created, end: verified.timestamp, deliveryId: delivery.snapshot.deliveryId })
  }
  const deploymentDelay = intervals.length
    ? availability({
        value: {
          semantics: 'release_approved_to_successful_delivery_endpoint',
          intervals: intervals.map((item) => ({
            deliveryId: item.deliveryId,
            endpoint: item.endpoint,
            startAt: new Date(item.start).toISOString(),
            endAt: new Date(item.end).toISOString(),
    ...
    : availability({
        complete: false,
        reasons: reasons.length ? reasons : ['delivery_promotion_proof_unavailable'],
        sources,
      })
  const leadReasons =
    deliveries.filter((item) => includedIds.has(item.snapshot?.workflowId)).length === leadIntervals.length
      ? []
      : ['authoritative_creation_or_production_verification_unavailable']
  const lead = leadIntervals.length
    ? availability({
        value: {
          semantics: 'workflow_created_to_production_verified',
          intervals: leadIntervals.map((item) => ({
            deliveryId: item.deliveryId,
            startAt: new Date(item.start).toISOString(),
            endAt: new Date(item.end).toISOString(),
            durationMs: item.end - item.start,
        ...
        complete: leadReasons.length === 0,
        reasons: leadReasons,
        sources,
      })
    : unavailable('authoritative_creation_or_production_verification_unavailable', sources)
  return { deploymentDelay, workflowCreatedToProductionVerified: lead }
}

export function projectFactoryOperationalMetrics({
  ...
  if (!['self', 'descendants', 'selection'].includes(scope))
    throw new TypeError('scope must be self, descendants, or selection')
  ...
  for (const snapshot of snapshots) {
    ...
    seen.add(id)
    items.push({ workflowId: id, snapshot, timing: timingsByWorkflowId[id] })
  }
  if (!items.length)
    return {
      schemaVersion: '1',
      observedAt: new Date(instant(observedAt)).toISOString(),
      scope: { kind: scope, workflowId: rootWorkflowId, includedWorkflowIds: [] },
      metrics: {
        cycleTime: unavailable('workflow_not_found', ['workflow-journal']),
        reviewTime: unavailable('workflow_not_found', ['human-interaction-journal']),
        currentWip: unavailable('workflow_not_found', ['workflow-snapshot']),
        deploymentDelay: unavailable('workflow_not_found', ['delivery-operation-journal']),
        workflowCreatedToProductionVerified: unavailable('workflow_not_found', ['delivery-operation-journal']),
      },
  ...
  return {
    schemaVersion: '1',
    observedAt: new Date(instant(observedAt)).toISOString(),
    scope: { kind: scope, workflowId: rootWorkflowId, includedWorkflowIds: [...includedIds].sort() },
    metrics: {
      cycleTime: cycleTime(items, scope),
      reviewTime: reviewTime(filteredInteractions, observedAt),
      currentWip: currentWip(items),
      ...delivery,
    },
    capabilities: unsupportedCapabilities(),
  }
