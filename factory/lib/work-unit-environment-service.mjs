...
const machine = (e) => (typeof e?.code === 'string' && /^[A-Z][A-Z0-9_]{0,63}$/.test(e.code) ? e.code : 'GIT_FAILED')
const sameIdentity = (environment, input, id) =>
...
export class WorkUnitEnvironmentService {
  constructor({ store, git, clock = () => new Date(), idGenerator = () => randomUUID(), fault = async () => {} }) {
    this.store = store
    this.git = git
    this.clock = clock
  ...
  async _provision(input, id) {
    ...
    if (current) {
      ...
      if (reconciliation.status === 'owned') {
        ...
        return { ok: true, changed: false, snapshot: current, headCommit: reconciliation.headCommit }
      }
