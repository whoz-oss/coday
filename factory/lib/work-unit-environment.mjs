...
export function validateNamespaceId(value, path = 'namespaceId') {
  return typeof value === 'string' && UUID.test(value)
    ? { ok: true }
    : fail(WORK_UNIT_ENVIRONMENT_ERROR_CODES.INVALID_UUID, path)
}
export function validateCanonicalAbsolutePath(value, path = 'path') {
...
export function validateGitRef(value, path = 'ref') {
...
export function validateWorkUnitEnvironment(input) {
  ...
  for (const field of ['environmentId', 'workUnitId', 'createdBy']) {
    const result = safe(input[field], field)
    if (!result.ok) return result
  }
  const workflow = safe(input.workflowId, 'workflowId', true)
  if (!workflow.ok) return workflow
  for (const field of ['businessRef', 'businessType']) {
    const result = safe(input[field], field, true)
    if (!result.ok) return result
  }
  const namespace = validateNamespaceId(input.namespaceId)
  if (!namespace.ok) return namespace
  if (input.parentCaseId !== undefined && !UUID.test(input.parentCaseId))
    return fail(WORK_UNIT_ENVIRONMENT_ERROR_CODES.INVALID_UUID, 'parentCaseId')
  for (const field of ['repoRoot', 'worktreePath']) {
    const result = validateCanonicalAbsolutePath(input[field], field)
    if (!result.ok) return result
  }
  ...
  for (const field of ['integrationBranch', 'branch']) {
    const result = validateGitRef(input[field], field)
    if (!result.ok) return result
  }
  if (input.baseCommit !== null && !SHA.test(input.baseCommit ?? ''))
    return fail(WORK_UNIT_ENVIRONMENT_ERROR_CODES.INVALID_SHA, 'baseCommit')
  if (input.baseCommit === null && input.lifecycleState !== 'provisioning')
    return fail(WORK_UNIT_ENVIRONMENT_ERROR_CODES.INVALID_SHA, 'baseCommit')
  ...
  if (input.lifecycleState === 'active' && !input.parentCaseId)
    return fail(WORK_UNIT_ENVIRONMENT_ERROR_CODES.INVALID_STATE, 'parentCaseId')
  if (input.lifecycleState === 'provisioning' && input.parentCaseId)
    return fail(WORK_UNIT_ENVIRONMENT_ERROR_CODES.INVALID_STATE, 'parentCaseId')
  const environment = {
    schemaVersion: '1',
