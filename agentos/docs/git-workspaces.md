...
`retry` belongs to the root case and accepts `acknowledgeSetupReplay` (default false). Execution recovery is available from each case. Deletion continues to use the existing Case DELETE endpoint.
