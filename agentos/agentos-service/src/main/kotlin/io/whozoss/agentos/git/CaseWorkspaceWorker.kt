...
class CaseWorkspaceWorker(
    ...
    fun provisionPending() = submitWorkspaceSweep(executor, sweeping) { provisionSweep() }
    ...
    private fun prepareRequestedCheckouts() {
        ...
        requested.forEach { checkout ->
            ...
            try {
                ...
                logger.error { "Could not prepare the checkout of namespace ${checkout.namespaceId} (${e.javaClass.simpleName})" }
            }
        }
    }
    ...
    private fun provisionLocked(requested: CaseResourceBinding): Boolean {
        ...
        try {
            ...
            logger.error { "Could not provision workspace ${binding.id} for case ${binding.rootCaseId} (${e.javaClass.simpleName})" }
            return false
        }
    }
