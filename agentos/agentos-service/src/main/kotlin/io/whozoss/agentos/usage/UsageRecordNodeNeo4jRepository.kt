...
interface UsageRecordNodeNeo4jRepository : Neo4jRepository<UsageRecordNode, String> {
    ...
    //   alongside the partial sum. The Kotlin caller (Neo4jUsageRecordRepository) replaces
    //   the partial sum with null when nullCostCount > 0.
    //
     ...
     *
     * Returns a single-column list with one map containing: recordCount, inputTokens,
     * outputTokens, cacheReadTokens, cacheWriteTokens, totalTokens, partialCostSum,
     * nullCostCount.
     */
    @Transactional(readOnly = true)
    @Query(
