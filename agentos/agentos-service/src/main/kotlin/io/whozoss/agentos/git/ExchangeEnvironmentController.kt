...
class ExchangeEnvironmentController(
    ...
    fun caseEnvironment(@PathVariable caseId: UUID): ExchangeEnvironment {
        val root = authorizedRoot(caseId)
    ...
    private fun authorizedRoot(caseId: UUID): GitExchangeRoot = roots.resolveGit(caseId).also {
        capabilities.requireCaseAccess(users.getCurrentUser().id.toString(), caseId, it.exchange, Action.READ)
    }
