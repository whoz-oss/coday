...
class FactoryGetWorkflowTool(
    ...
    override val inputSchema = """{"type":"object","additionalProperties":false,"properties":{"workflowId":{"type":"string","pattern":"^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$"}},"required":["workflowId"]}"""
    ...
    override suspend fun execute(input: Input?, context: ToolContext): ToolExecutionResult {
        ...
        val request = Request.Builder().url("${baseUrl.trimEnd('/')}/api/factory/workflows/$encoded?namespaceId=${context.namespaceId}").get().build()
        return try {
            withContext(Dispatchers.IO) { httpClient.newCall(request).execute().use { parseResponse(workflowId, it.code, it.body?.string()) } }
    ...
    internal fun parseResponse(workflowId: String, status: Int, body: String?): ToolExecutionResult {
        ...
        if (status !in 200..299) {
            ...
            val message = root.path("error").path("message").takeIf { it.isTextual }?.asText() ?: "Factory rejected the lookup."
            return failure(code, message)
        }
        val data = root.path("data")
        val state = data.path("state").takeIf { it.isTextual }?.asText()
        if (data.path("workflowId").asText(null) != workflowId || state !in STATES) return failure("MALFORMED_FACTORY_RESPONSE", "Factory returned an invalid response.")
        val output = if (state != "existing") mapOf("state" to state, "workflowId" to workflowId) else {
            ...
            if (revision == null || projection == null || projection.path("workflowId").asText(null) != workflowId || workflowType == null) return failure("MALFORMED_FACTORY_RESPONSE", "Factory returned an invalid response.")
            mapOf("state" to state, "workflowId" to workflowId, "revision" to revision, "workflowType" to workflowType, "status" to projection.path("status").asText(), "projection" to projection)
        }
