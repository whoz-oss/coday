...
interface PermissionNodeNeo4jRepository : Neo4jRepository<UserNode, String> {
    ...
    @Query(
        $$"""
        MATCH (u:User {id: $userId, removed: false})-[r:ADMIN|MEMBER]->(e {id: $entityId})
    ...
    @Query(
        $$"""
        MATCH (u:User {id: $userId, removed: false})-[r:ADMIN]->(e {id: $entityId})
        WHERE $entityLabel IN labels(e)
        RETURN COUNT(r) > 0
    """,
    )
    ...
    @Query(
        $$"""
        MATCH (u:User {id: $userId, removed: false})-[:ADMIN]->(n:Namespace {removed: false})
    ...
    @Query(
        $$"""
        MATCH (u:User {id: $userId, removed: false})-[:ADMIN|MEMBER]->(n:Namespace {removed: false})
        MATCH (n)<-[:BELONGS_TO]-(e {id: $entityId})
        WHERE $entityLabel IN labels(e)
        RETURN COUNT(e) > 0
    """,
    ...
    @Query(
        $$"""
        MATCH (u:User {id: $userId, removed: false})-[r:MEMBER]->(e {id: $entityId})
        WHERE $entityLabel IN labels(e)
        DELETE r
    """,
    )
    ...
    @Query(
        $$"""
        MATCH (u:User {id: $userId, removed: false})-[r:ADMIN]->(e)
        WHERE $entityLabel IN labels(e)
        RETURN e.id
    """,
    )
    ...
    @Query(
        $$"""
        MATCH (u:User {id: $userId, removed: false})-[r:ADMIN|MEMBER]->(e)
        WHERE $entityLabel IN labels(e)
        RETURN e.id
    """,
    )
    ...
    @Query(
        $$"""
        MATCH (u:User {id: $userId, removed: false})-[:ADMIN]->(n:Namespace {removed: false})<-[:BELONGS_TO]-(e)
        WHERE $entityLabel IN labels(e)
        RETURN e.id
        UNION
        MATCH (u:User {id: $userId, removed: false})-[:ADMIN]->(e)
        WHERE $entityLabel IN labels(e)
        RETURN e.id
    """,
    )
    ...
    @Query(
        $$"""
        MATCH (u:User {id: $userId, removed: false})-[:ADMIN|MEMBER]->(n:Namespace {removed: false})<-[:BELONGS_TO]-(e)
        WHERE $entityLabel IN labels(e)
        RETURN e.id
        UNION
        MATCH (u:User {id: $userId, removed: false})-[:ADMIN|MEMBER]->(e)
        WHERE $entityLabel IN labels(e)
        RETURN e.id
    """,
    )
    ...
    @Query(
        ...
        WHERE $entityLabel IN labels(e)
          ...
          AND (
            ...
            OR EXISTS { (e)-[:BELONGS_TO]->(n:Namespace{removed: false})<-[:ADMIN|MEMBER]-(u) }
          )
        RETURN e.id
    """,
    )
    ...
    @Query(
        ...
        WHERE $entityLabel IN labels(e)
          ...
          AND (
            ...
            OR EXISTS { (u)-[:ADMIN]->(e) }
            OR EXISTS { (e)-[:BELONGS_TO]->(n:Namespace{removed: false})<-[:ADMIN]-(u) }
          )
        RETURN e.id
    """,
    )
