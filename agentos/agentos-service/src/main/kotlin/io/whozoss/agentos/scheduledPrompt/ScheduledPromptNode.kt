...
data class ScheduledPromptNode(
    ...
    companion object {
        ...
        fun fromDomain(scheduledPrompt: ScheduledPrompt): ScheduledPromptNode =
            ScheduledPromptNode(
                id = scheduledPrompt.id.toString(),
                namespaceId = scheduledPrompt.namespaceId?.toString(),
                userId = scheduledPrompt.userId?.toString(),
                agentConfigId = scheduledPrompt.agentConfigId.toString(),
                promptTemplateId = scheduledPrompt.promptTemplateId.toString(),
                name = scheduledPrompt.name,
                description = scheduledPrompt.description,
                unit = scheduledPrompt.recurrence.unit.name,
                days = scheduledPrompt.recurrence.days.map { it.name },
                timeUtc = scheduledPrompt.recurrence.timeUtc,
                startDate = scheduledPrompt.planning.startDate,
                endType = scheduledPrompt.planning.endType.name,
                endDate = scheduledPrompt.planning.endDate,
                maxOccurrenceCount = scheduledPrompt.planning.maxOccurrenceCount,
                enabled = scheduledPrompt.enabled,
                nextRunAt = scheduledPrompt.nextRunAt,
                lastRunAt = scheduledPrompt.lastRunAt,
                tripleKey = computeTripleKey(scheduledPrompt.namespaceId, scheduledPrompt.userId, scheduledPrompt.name),
                version = scheduledPrompt.metadata.version,
                created = scheduledPrompt.metadata.created,
                createdBy = scheduledPrompt.metadata.createdBy,
