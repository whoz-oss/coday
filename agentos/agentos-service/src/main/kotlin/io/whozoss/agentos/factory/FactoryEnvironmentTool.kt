...
class FactoryProvisionEnvironmentTool(
    ...
    override val inputSchema = """{"type":"object","additionalProperties":false,"properties":{"workflowId":{"type":"string"},"workUnitId":{"type":"string"},"integrationBranch":{"type":"string"},"branch":{"type":"string"}},"required":["workflowId","workUnitId","integrationBranch","branch"]}"""
    ...
    override suspend fun execute(input: Input?, context: ToolContext): ToolExecutionResult {
        ...
        } catch (_: Exception) { failure("FACTORY_UNAVAILABLE", "Factory is unavailable.") }
    }

    internal fun parse(status: Int, body: String?): ToolExecutionResult {
        ...
        if (status !in 200..299) return failure(root.path("error").path("code").asText("ENVIRONMENT_NOT_BOUND"), root.path("error").path("message").asText("Environment was not bound."))
        val data = root.path("data")
        if (data.path("fileAccess").path("status").asText() != "bound") return failure("ENVIRONMENT_NOT_BOUND", "Factory did not prove FILE_ACCESS binding.")
        val environment = data.path("environment")
        if (environment.path("workflowId").asText().isBlank() || environment.path("lifecycleState").asText() != "active") return failure("MALFORMED_FACTORY_RESPONSE", "Factory returned an invalid environment binding.")
        val safeOutput = mapOf("environmentId" to environment.path("environmentId").asText(), "workflowId" to environment.path("workflowId").asText(), "workUnitId" to environment.path("workUnitId").asText(), "lifecycleState" to "active", "baseCommit" to environment.path("baseCommit").asText(), "headCommit" to data.path("headCommit").asText(), "fileAccessStatus" to "bound")
        return ToolExecutionResult.success(objectMapper.writeValueAsString(safeOutput), metadata = mapOf("environmentId" to environment.path("environmentId").asText(), "status" to "bound"))
    }
