...
class ExchangeGitDiff(private val runner: GitCommandRunner) {
    ...
    fun changes(target: ExchangeGitTarget): ExchangeGitChanges {
        ...
        val names = checked(run(target, diffArgs + listOf("--name-status", "-z", base, "--")))
            .split('\u0000').dropLast(1)
        ...
        val conflicted = checked(run(target, diffArgs + listOf("--name-only", "--diff-filter=U", "-z", "--")))
            .split('\u0000').filter { it.isNotEmpty() }.toSet()
        val tracked = checked(run(target, diffArgs + listOf("--numstat", "-z", base, "--")))
        val files = tracked.split('\u0000').filter { it.isNotEmpty() }.map { row ->
        ...
        if (untracked.size + files.size > 5000) throw ConflictException("Too many changed files to display (limit: 5000)")
