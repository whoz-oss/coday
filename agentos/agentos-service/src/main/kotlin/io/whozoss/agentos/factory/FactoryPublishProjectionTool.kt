...
class FactoryPublishProjectionTool(
    ...
    override suspend fun execute(input: Input?, context: ToolContext): ToolExecutionResult {
        ...
        val agentName = context.agentName?.takeIf { it.isNotBlank() }
            ?: return failure("AGENT_CONTEXT_UNAVAILABLE", "Agent identity is required.")
        val userIdentity = context.userExternalId?.takeIf { it.isNotBlank() } ?: context.userId?.toString()
    ...
    internal fun projectionPayload(input: Input): Map<String, Any> = buildMap {
        ...
        put("steps", input.steps.map { step ->
            buildMap<String, Any> {
                put("id", step.id)
                put("name", step.name)
    ...
    internal fun parseResponse(status: Int, body: String?): ToolExecutionResult {
        ...
        if (status in 200..299) {
            ...
            if (workflowId == null || revision == null || changed == null) return failure("MALFORMED_FACTORY_RESPONSE", "Factory returned an invalid response.")
