package io.biznet.agentos.entity
...
abstract class EntityController<EntityType : Entity, ParentIdentifier>(
     ...
     */
    @GetMapping("/{id}")
    fun getById(@PathVariable id: UUID): EntityType {
     ...
     */
    @GetMapping
    fun getByIds(@RequestParam(required = false) ids: List<UUID>?): List<EntityType> {
     ...
     */
    protected fun listByParent(parentId: ParentIdentifier): List<EntityType> {
    ...
    fun create(@RequestBody entity: EntityType): EntityType {
     ...
     */
    @DeleteMapping
    fun deleteMany(@RequestParam ids: List<UUID>): Map<String, Int> {
