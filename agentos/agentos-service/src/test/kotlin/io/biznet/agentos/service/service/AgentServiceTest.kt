...
class AgentServiceTest : StringSpec({
    ...
    "listAgents should return metadata from all plugins" {
        val mockAgent1 = mockk<AgentPlugin> {
            every { getMetadata() } returns AgentMetadata(
                name = "agent1",
        ...
        val mockAgent2 = mockk<AgentPlugin> {
            every { getMetadata() } returns AgentMetadata(
                name = "agent2",
    ...
    "executeAgent should call correct agent" {
        val mockAgent = mockk<AgentPlugin> {
            every { getMetadata() } returns AgentMetadata(
                name = "test-agent",
                description = "Test",
                version = "1.0.0"
            )
            coEvery { execute(any()) } returns AgentOutput(
