...
class AiProviderControllerSpec :
    StringSpec({
        ...
        "toDto masks a long apiKey" {
            toDto(config(apiKey = "sk-ant-api03-abcdefghijklmnop")).apiKey shouldBe "sk-a****mnop"
        }

        "toDto returns null apiKey when no key is set" {
            toDto(config(apiKey = null)).apiKey.shouldBeNull()
        }

        "toDto maps namespaceId and userId" {
            val uid = UUID.randomUUID()
            val r = toDto(config(nsId = namespaceId, uId = uid))
            r.namespaceId shouldBe namespaceId
            r.userId shouldBe uid
        }

        "toDto maps all fields and masks apiKey" {
        ...
        "toDto with null apiKey returns null apiKey" {
            toDto(config(apiKey = null)).apiKey shouldBe null
        }
