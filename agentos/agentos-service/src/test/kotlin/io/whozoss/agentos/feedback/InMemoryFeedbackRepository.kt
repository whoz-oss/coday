...
class InMemoryFeedbackRepository : FeedbackRepository {
    ...
    private val delegate = InMemoryEntityRepository<Feedback, UUID>(
    ...
    override fun findByCaseEventId(caseEventId: UUID): List<Feedback> =
        delegate.findAll().filter { it.caseEventId == caseEventId }

    override fun upsert(entity: Feedback, userId: String?): Feedback {
        val existing = userId?.let { uid ->
