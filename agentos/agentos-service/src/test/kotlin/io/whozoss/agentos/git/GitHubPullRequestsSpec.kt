...
class GitHubPullRequestsSpec : StringSpec({
    ...
    fun fixture(vararg responses: Pair<Int, String>, secret: String = "test-token"): Fixture {
        val requests = mutableListOf<HttpRequest>()
        val client = mockk<HttpClient>()
        val accounts = mockk<GitServiceAccountResolver> {
            every { resolve(settings) } returns GitCredentials.UsernamePassword("service-account", secret)
        }
    ...
    "invalid HTTP credentials never survive in an exception or cause" {
        listOf("synthetic-token\n", "synthetic-token\u0100").forEach { secret ->
            val f = fixture(secret = secret)
            val error = shouldThrow<IllegalArgumentException> { f.provider.inspect(settings, "local-review", headSha) }
