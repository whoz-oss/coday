...
/**
 * Unit tests for the [AgentosPluginConfiguration] lambdas.
 *
 * Instantiates [AgentosPluginConfiguration] directly (no Spring context), wires it
 * with in-memory repositories and MockK permission services, then exercises each tool
 * through [AgentosToolPlugin.provideTools] → tool.execute().
 *
 * This approach tests the **actual** lambda body from [AgentosPluginConfiguration],
 * not a hand-written copy, so any weakened guard is caught here.
 */
class AgentosPluginConfigurationUnitSpec : StringSpec({
    ...
    fun buildPlugin(
        ...
        val config = AgentosPluginConfiguration(repo, permissionService)
        return config.agentosToolPlugin() as AgentosToolPlugin
    }

    fun context(uid: UUID? = userId) = ToolContext(
