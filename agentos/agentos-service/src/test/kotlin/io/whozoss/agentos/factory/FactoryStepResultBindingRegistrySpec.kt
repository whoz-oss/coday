...
class FactoryStepResultBindingRegistrySpec : StringSpec({
 "binding is case namespace agent scoped, bounded and volatile" {
  val now=Instant.parse("2030-01-01T00:00:00Z");val registry=FactoryStepResultBindingRegistry(Clock.fixed(now,ZoneOffset.UTC));val caseId=UUID.randomUUID();val ns=UUID.randomUUID()
 ...
 "binding survives initial idle lifecycle and remains available for the bound turn" {
  ...
  // IDLE is deliberately not a registry operation: CaseService must not remove
  // an unconsumed binding during the create→message→RUNNING lifecycle window.
  registry.contains(caseId) shouldBe true
