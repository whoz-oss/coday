 ...
 */
class AgentConfigControllerUnitSpec :
    StringSpec({
        ...
        "toDto returns null createdBy and updatedBy when EntityMetadata has none" {
        ...
        "toDomain maps all fields from AgentConfigDto to AgentConfig" {
            val id = UUID.randomUUID()
            val r =
                resource(
                    id = id,
        ...
        "getByIds returns all matching entities for a super-admin caller (admin bypass)" {
            ...
            val result =
                controller.getByIds(
                    GetByIdsRequest(
                        ids = listOf(c1.id, c2.id),
                    ),
                )

            result shouldBe listOf(toDto(c1), toDto(c2))
        }
        ...
        "getByIds filters via permissionService.filterVisibleIds for a regular caller" {
            ...
            every {
                permissionService.filterVisibleIds(
                    ...
                    listOf(c1.id.toString(), c2.id.toString()),
                    Action.READ,
                )
            } returns setOf(c1.id.toString())
            // Delegate fetches the full input set then filters in-process:
            every { service.findByIds(setOf(c1.id, c2.id), false) } returns listOf(c1, c2)

            val result =
                controller.getByIds(
                    GetByIdsRequest(ids = listOf(c1.id, c2.id)),
                )

            result shouldBe listOf(toDto(c1))
        }
        ...
        "getByIds returns empty list for a regular caller with no visible ids (without hitting service.findByIds)" {
            ...
            // Delegate still fetches all requested IDs even when filterVisibleIds returns empty:
        ...
        "listByParent returns configs for the given namespaceId (withDisabled=true by default)" {
        ...
        "listPlatformAgents default (withDisabled=false) returns only enabled platform configs" {
            ...
            result shouldBe listOf(toDto(p1))
        ...
        "listPlatformAgents with withDisabled=true returns all platform configs including disabled" {
            ...
            result shouldBe listOf(toDto(p1), toDto(p2))
        ...
        "update delegates to service when entity exists and returns mapped resource" {
            ...
            val updatedDomain = toDomain(updatedResource)
            every { userService.getCurrentUser() } returns superAdmin
            ...
            result shouldBe toDto(updatedDomain)
