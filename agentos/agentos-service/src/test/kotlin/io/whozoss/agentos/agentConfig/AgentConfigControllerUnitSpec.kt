...
class AgentConfigControllerUnitSpec :
    StringSpec({
        ...
        "getDeployments returns list of DeploymentScope from service" {
            val id = UUID.randomUUID()
            val nsId = UUID.randomUUID()
            val expected =
                listOf(
                    io.whozoss.agentos.entity.DeploymentScope(
                        scopeType = io.whozoss.agentos.entity.ScopeType.NAMESPACE,
                        id = nsId,
                    ),
                )
            every { service.findDeployments(id) } returns expected

            val result = controller.getDeployments(id)

            result shouldBe expected
            verify(exactly = 1) { service.findDeployments(id) }
        }

        "getDeployments returns empty list when agent has no deployments" {
            val id = UUID.randomUUID()
            every { service.findDeployments(id) } returns emptyList()

            controller.getDeployments(id) shouldBe emptyList()
        }
