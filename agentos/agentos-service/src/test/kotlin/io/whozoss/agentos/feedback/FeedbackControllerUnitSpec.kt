...
class FeedbackControllerUnitSpec :
    StringSpec({
        ...
        "create resolves both caseId and namespaceId from the fetched CaseEvent" {
            // Neither caseId nor namespaceId is in FeedbackInput — both must come from the event.
            val serverNamespaceId = UUID.randomUUID()
            val serverCaseId = UUID.randomUUID()
            val capturedCaseId = mutableListOf<UUID>()
            val capturedNamespaceId = mutableListOf<UUID>()
            every { caseEventService.findById(caseEventId) } returns
                MessageEvent(
                    metadata = EntityMetadata(),
            ...
            every { feedbackService.upsert(any()) } answers {
                ...
                capturedCaseId += f.caseId
                capturedNamespaceId += f.namespaceId
                f
            }

            controller.create(input())

            capturedCaseId.first() shouldBe serverCaseId
            capturedNamespaceId.first() shouldBe serverNamespaceId
        }
