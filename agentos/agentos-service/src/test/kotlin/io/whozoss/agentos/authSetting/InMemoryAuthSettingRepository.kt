...
class InMemoryAuthSettingRepository : AuthSettingRepository {
    private val delegate =
        InMemoryEntityRepository<AuthSetting, String>(
    ...
    override fun findByIds(
        ids: Collection<UUID>,
        withRemoved: Boolean,
    ): List<AuthSetting> = delegate.findByIds(ids, withRemoved)
    ...
    override fun findByUserId(userId: UUID): List<AuthSetting> = delegate.findAll().filter { it.userId == userId }
    ...
    override fun findPlatformLevel(): List<AuthSetting> = delegate.findAll().filter { it.namespaceId == null && it.userId == null }
