...
class ScheduledPromptControllerIntegrationSpec : StringSpec() {
    ...
    private fun postSp(body: String = validPayload) =
    ...
    init {
        ...
        "PUT with blank name returns 400" {
            mockMvc.perform(
                put("/api/scheduled-prompts/${UUID.randomUUID()}")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("""
                        {
                            "agentConfigId": "$agentConfigId",
        ...
        "DELETE on non-existent id returns 404" {
            mockMvc.perform(delete("/api/scheduled-prompts/${UUID.randomUUID()}"))
                .andExpect(status().isNotFound)
        }
        ...
        "POST /by-ids with valid body returns 200" {
            mockMvc.perform(
                post("/api/scheduled-prompts/by-ids")
                    .contentType(MediaType.APPLICATION_JSON)
                    .content("""{"ids": []}"""),
            ).andExpect(status().isOk)
        }
