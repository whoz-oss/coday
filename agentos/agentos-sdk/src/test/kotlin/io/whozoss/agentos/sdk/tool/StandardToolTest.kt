 ...
 */
class StandardToolTest : StringSpec({

    // Minimal tool implementation for unit-testing the default executeWithJson behaviour.
    data class TimezoneInput(val timezone: String = "UTC")

    val testTool = object : StandardTool<TimezoneInput> {
        ...
        override fun execute(input: TimezoneInput?): String =
            input?.timezone ?: "null-input"
    }
    ...
    "executeWithJson with explicit timezone uses that value" {
        testTool.executeWithJson("\"{\\\"timezone\\\":\\\"America/New_York\\\"}\"".let {
