 ...
 * endpoint before an [io.whozoss.agentos.sdk.caseEvent.AnswerEvent] is persisted.
 *
 * This client is intentionally free of framework annotations so it can be constructed
 * inside the plugin classloader and reused by the [FactoryAnswerInterceptor].
 *
 * ## Trust model
 *
 ...
 *
 * The caller must NOT persist an [io.whozoss.agentos.sdk.caseEvent.AnswerEvent] on failure.
 */
open class FactoryCheckpointClient(
     ...
     *
     * @param ref The checkpoint reference embedded in the
     *   [io.whozoss.agentos.sdk.caseEvent.QuestionEvent].
            ...
            withContext(Dispatchers.IO) {
                httpClient.newCall(request).execute().use { response ->
                    ...
                    if (response.isSuccessful) {
                        logger.info {
                            "Factory checkpoint accepted: workflow=${ref.workflowId} interaction=${ref.interactionId} decision=$decision"
                        }
                        Result.success(Unit)
                    } else {
                        val errorCode =
                        ...
                        val errorMessage =
                            runCatching {
                                ...
                                ?: "Factory rejected the decision (HTTP ${response.code})"
                        logger.warn {
                            "Factory checkpoint rejected: workflow=${ref.workflowId} interaction=${ref.interactionId} code=$errorCode message=$errorMessage"
                        }
 ...
 * @param message Human-readable explanation surfaced to the user.
 */
