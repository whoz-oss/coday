...
internal object FactoryProjectionValidation {
    ...
    fun validate(input: FactoryPublishProjectionTool.Input?): Failure? {
        ...
        input.steps.forEachIndexed { index, step ->
            ...
            if (step.description?.let { !bounded(it, maxDescription) } == true) {
                return Failure("INVALID_PROJECTION", "$field.description", "must be non-blank and at most $maxDescription characters")
            }
            ...
            if (input.schemaVersion == "1" && step.responsibility != null) {
                return Failure("INVALID_PROJECTION", "$field.responsibility", "is not allowed in schema v1")
            }
            if (input.schemaVersion == "2") {
                ...
                if (actor.name?.let { !bounded(it, maxText) } == true) {
                    return Failure("INVALID_PROJECTION", "$field.responsibility.name", "must be non-blank and at most $maxText characters")
                }
            }
        }
        input.steps.forEachIndexed { index, step ->
    ...
    private fun bounded(
        value: String,
        maximum: Int,
    ) = value.isNotBlank() && value.length <= maximum
}
