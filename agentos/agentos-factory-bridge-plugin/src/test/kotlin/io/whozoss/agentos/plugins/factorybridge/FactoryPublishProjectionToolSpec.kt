    ...
    "rejects the smoke payload's in_progress step status with actionable feedback" {
        ...
        val context =
            ToolContext(
                ...
                UUID.randomUUID(),
                "user",
                listOf(CaseStatusEvent(metadata = EntityMetadata(), namespaceId = namespace, caseId = UUID.randomUUID(), status = CaseStatus.PENDING)),
                "agent",
            )
