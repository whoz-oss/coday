...
export class GitTools extends AssistantToolFactory {
  ...
  protected async buildTools(context: CommandContext, agentName: string): Promise<CodayTool[]> {
    ...
    const gitStatusTool: FunctionTool<unknown> = {
      type: 'function',
      function: {
        name: 'gitStatusTool',
    ...
    const gitDiffTool: FunctionTool<unknown> = {
      type: 'function',
      function: {
        name: 'gitDiffTool',
    ...
    const gitAddTool: FunctionTool<{ filePaths: string[] }> = {
      ...
      function: {
        name: 'gitAddTool',
        description:
          'DO NOT USE UNLESS EXPLICITLY ASKED TO.Add files to staging area. Please add only relevant files and check with git status before.',
        parameters: {
          type: 'object',
          properties: {
            filePaths: {
    ...
    const gitCommitTool: FunctionTool<{ message: string }> = {
      ...
      function: {
        name: 'gitCommitTool',
        description:
          'DO NOT USE UNLESS EXPLICITLY ASKED TO. Commit changes with a message. Write a short commit message starting with the id of the current task or ticket or subject. Add a short paragraph after if significant changes (ie not for typos or comments commits).',
        parameters: {
          type: 'object',
          properties: {
            message: {
    ...
    const gitListBranchesTool: FunctionTool<unknown> = {
      type: 'function',
      function: {
    ...
    const gitCreateBranchTool: FunctionTool<{ branchName: string; baseBranch?: string }> = {
      ...
      function: {
        ...
        description:
          'DO NOT USE UNLESS EXPLICITLY ASKED TO. Create a new branch from the head of another branch or the current branch.',
        parameters: {
          type: 'object',
          properties: {
            branchName: {
    ...
    const gitLogTool: FunctionTool<{ params: string }> = {
      ...
      function: {
        name: 'gitLogTool',
        description:
          'run git log command to list commits made in the repository. The params parameter allows you to pass additional arguments to the git log command.',
        parameters: {
          type: 'object',
          properties: {
            params: {
