  ...
  async handle(command: string, context: CommandContext): Promise<CommandContext> {
    ...
    if (providerStart) {
      ...
      const status: string = [
        ...
        ...AI_PROVIDERS.map((provider) => {
          ...
          return `${userSet ? '✅' : '❌'} ${provider}${projectSet ? ' (project default available)' : ''}`
        }),
      ].join('\n')
    ...
    if (apiKeyFromCommand) {
      ...
      const apiKeyPrompt = currentApiKey
        ? `Enter API key for ${provider} (Current value is masked, validate to leave unchanged${projectHasKey ? ', project default available' : ''})`
