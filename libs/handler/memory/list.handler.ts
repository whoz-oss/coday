...
export class MemoryListHandler extends CommandHandler {
  constructor(
    ...
    super({
      commandWord: 'list',
      description: 'List memories. Optional filters: [PROJECT|USER] and agent:AgentName (e.g., "memory list USER" or "memory list agent:Sway")',
    })
  }
  ...
  async handle(command: string, context: CommandContext): Promise<CommandContext> {
    ...
    // Build output string
    let outputText = `Found ${memories.length} memories` +
    ...
    if (!showFullDetails) {
      ...
      outputText += "Tip: Use filters like 'memory list PROJECT', 'memory list USER', or 'memory list agent:agentName' for fewer results and more details.\n"
      outputText += "For full content, refine your search or (future) use pagination options.\n"
    }

    this.interactor.displayText(outputText)
    return context
  }
}
