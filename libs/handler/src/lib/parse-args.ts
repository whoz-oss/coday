export type ParseResult = Record<string, string | boolean> & { rest: string }
...
export function parseArgs(command: string, inputs: KeyInput[]): ParseResult {
  const SEPARATOR = ' '
  const splited = command.split(SEPARATOR).filter((v) => !!v)
  const result: ParseResult = { rest: '' }
