...
/**
 * IntegrationConfigExportService — hand-written wrapper for the YAML export endpoint.
 *
 * `GET /api/integration-configs/{id}/export` returns `application/yaml` exclusively.
 * The generated `IntegrationConfigControllerService.exportIntegrationConfig()` cannot
...
export class IntegrationConfigExportService {
  private readonly http = inject(HttpClient)
  private readonly config = inject(Configuration)

  /**
   * Fetch a single integration config as a YAML string.
   * The returned string is the raw YAML, ready to be saved as a `.yaml` file.
   */
  exportAsYaml(id: string): Observable<string> {
    const url = `${this.config.basePath}/api/integration-configs/${encodeURIComponent(id)}/export`
