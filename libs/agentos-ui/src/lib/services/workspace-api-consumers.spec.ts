...
describe('Workspace API consumers', () => {
  ...
  it('decodes workspace JSON through the generated client', () => {
    const view = { equipped: true, recoveryRequired: false, rootCaseId: 'root', status: 'READY' }
  ...
  it('decodes namespace workspace lists before updating PR badges', fakeAsync(() => {
    ...
    expect(request.request.responseType).toBe('json')
    request.flush([{ equipped: true, recoveryRequired: false, rootCaseId: 'root', status: 'READY' }])
  ...
  it.each<WorkspaceAction>(['refresh', 'recover', 'retry'])(
    ...
    (action) => {
      ...
      request.flush({ equipped: true, recoveryRequired: false, rootCaseId: 'root' })
