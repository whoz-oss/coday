...
export class ExchangeStateService {
  ...
  private waitForWorkspace(
    ...
    return this.workspaces.get(caseId).pipe(
      switchMap((workspace) => {
        if (workspace.equipped && (workspace.status === 'REQUESTED' || workspace.status === 'PREPARING')) {
          return concat(
            of({ ...NEUTRAL_LOADING, status: 'preparing' as const }),
            timer(2000).pipe(switchMap(() => this.waitForWorkspace(caseId, loader)))
          )
        }
        if (!workspace.equipped || workspace.status === 'READY' || workspace.status === 'REMOVED') {
          // READY can arrive between the failed listing and the status check. Retry once;
          // any remaining file error must not start an endless preparation spinner.
