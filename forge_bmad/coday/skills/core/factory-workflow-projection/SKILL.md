...
1. Verify that `FACTORY__get_workflow` is available. Governed creation additionally requires `FACTORY__start_workflow`; `FACTORY__publish_projection` remains required only for historical declarative workflows. If either is absent, refuse to start a Factory run and explain which capability is required.
...
5. Call `FACTORY__get_workflow` with only the resolved `workflowId`. For `absent`, call `FACTORY__start_workflow` with only `workflowId`, `workflowType`, and `title`; Factory selects the unique immutable definition and derives the graph and initial states. For `existing` governed state, require matching type/definition and resume from the authoritative instance. For `existing` declarative state, preserve the complete-snapshot `FACTORY__publish_projection` path without implicit conversion. Any incompatibility is a hard refusal. Never ask the human for, infer, or guess a revision. For `removed` or `purged`, halt and report the lifecycle state; this protocol does not authorize restore, tombstone clearing, or identity replacement.
...
7. For a ready human-owned step, refresh then call `FACTORY__request_human_decision` with that exact revision, a bounded prompt, the fixed `approve` and `reject` actions, and an idempotency key. This is the only agent operation that opens the gate; it also moves the step to `waiting_human`. Stop after the call. Resume only after `FACTORY__get_workflow` reports the step `completed` or `failed`. Never submit a decision or auto-approve a human gate.
8. Represent waiting, blocked, failed, cancelled, and completed outcomes honestly. A failed terminal publication must be reported as a synchronization failure alongside the domain outcome.
