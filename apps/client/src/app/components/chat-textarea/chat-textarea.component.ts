...
export class ChatTextareaComponent implements OnInit, OnDestroy, AfterViewInit, OnChanges {
  ...
  onDocumentClick(event: MouseEvent): void {
    ...
    const popup = this.document.querySelector('.autocomplete-popup')
  ...
  private initializeVoiceInput(): void {
    ...
    const SpeechRecognition = (this.window as any).SpeechRecognition ?? (this.window as any).webkitSpeechRecognition
  ...
  private schedulePendingLineBreaks(): void {
    ...
    this.pendingLineBreaksTimeout = this.window.setTimeout(() => {
  ...
  private adjustTextareaHeight(): void {
    ...
    const style = this.window.getComputedStyle(textarea)
